...
@click.option('--model', '-m', 'model_file', multiple=True,
              type=click.Path(exists=True, dir_okay=False, resolve_path=True),
              required=True,
              help='The path to the file containing the ML model')
...
def main(geometry_file, parameters_file, model_file, output_file, scratch_dir):
    import run

    with open(parameters_file) as f:
        params = json.load(f)

    for g, o, m in zip(geometry_file, output_file, model_file):
        run.run_calculation(g, o, m, params, scratch_dir)
